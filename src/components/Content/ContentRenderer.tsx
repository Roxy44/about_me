...
function isContactItem(item: string | SkillItem | ContactListItem): item is ContactListItem {
...
function renderSkillItem(item: SkillItem, itemIndex: number, isMobile: boolean, measure?: string) {
    return (
        <li
            key={itemIndex}
        ...
        <div className={`flex flex-wrap ${isMobile ? 'justify-center gap-4' : 'items-start gap-6'}`}>
            {columns.map((columnItems, columnIndex) => (
                <ul key={columnIndex} className={`flex list-none flex-col gap-3 ${isMobile ? 'w-full' : ''}`}>
                    {columnItems.map((item, itemIndex) => renderSkillItem(item, itemIndex, isMobile, measure))}
                </ul>
            ))}
        </div>
    );
...
function renderBlock(
    ...
    switch (block.type) {
        case 'paragraph':
            return (
                <div key={index} className='space-y-2'>
                    {block.title && <h2 className='text-2xl font-bold'>{t(block.title)}</h2>}
        ...
        case 'heading':
            return (
                <div key={index} className='flex flex-wrap items-baseline gap-x-2 gap-y-1'>
                    <span className={`font-semibold ${isMobile ? 'text-lg' : 'text-xl'}`}>{block.text}</span>
                    {block.caption && <span className='text-sm italic text-gray-500'>{t(block.caption.main)}</span>}
                </div>
            );
            ...
            if (hasSkillItems) {
                return (
                    <div key={index} className='space-y-3'>
                        {renderSkillColumns(block.items ?? [], filterValue, block.columns ?? 3, isMobile, block.measure)}
                        {block.caption && <div className='text-sm italic text-gray-500'>{t(block.caption.main)}</div>}
                    </div>
                );
            }
            ...
            return (
                ...
                >
                    {(block.items ?? []).map((item, itemIndex) => {
                        ...
                        return (
                            <li key={itemIndex} className={block.dotList ? 'ml-6' : 'flex items-start gap-3'}>
                                {!block.dotList && <CircleCheck className='mt-1 size-5 shrink-0 text-green-700' />}
        ...
        case 'link':
            return (
                <p key={index}>
                    <a href={block.href} target='_blank' rel='noreferrer' className={linkClassName}>
                        {block.label}
                    </a>
                </p>
            );
        ...
        case 'sub-group':
            return (
                <div key={index}>
                    {block.title && (
                        <div
                            className={`mb-1 border-2 border-black-500 bg-blue-400 px-[12px] py-[4px] font-bold text-white ${isMobile ? 'text-xl text-center rounded-2xl' : 'w-fit text-2xl rounded-xl'}`}
                        >
                            {t(block.title)}
                        </div>
                    )}
                    ...
                    >
                        {block.isShort ? (
                            <ExpandList list={block.content ?? []} t={t} isMobile={isMobile} filterValue={filterValue} />
                        ) : (
...
function ContactsBlock({ block, t }: { block: ContentBlock; t: (text: string) => string }) {
...
function ToggleBlock({ block, t, isMobile }: { block: ContentBlock; t: (text: string) => string; isMobile: boolean }) {
    const [activeValue, setActiveValue] = useState(block.options?.[0]?.value ?? '');
    ...
    return (
        <div className='mt-4 flex flex-col gap-4'>
            ...
            {(block.content ?? []).map((child, childIndex) => renderBlock(child, childIndex, t, isMobile, activeValue))}
        </div>
    );
}
...
export function ContentRenderer({ content }: ContentRendererProps) {
    ...
    return <div className='space-y-4'>{content.map((block, index) => renderBlock(block, index, t, isMobile))}</div>;
}
